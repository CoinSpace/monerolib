...
const computeRound = (roundContext, memberIndex, scalar, challenge) => {
  const { preparedRing, muP, muC, keyImagePoint, auxiliaryImagePoint } = roundContext;
...
export function proveClsag(message, ring, pseudoOut, signer) {
  const { index, secretKey, commitmentScalar } = signer;
  const n = ring.length;
  ...
  return { s, c1: encodeInt(c1), I: keyImage, D: auxiliaryImage };
}
