...
import { hashToEc, hashToScalar, randomScalar } from './crypto-util.js';
import { invert, mod } from '@noble/curves/abstract/modular.js';

const L = CURVE.n;

// 1/8 mod l; sig.D stores D * INV_EIGHT, recovered on verify via scalarmult8 (D_8 = 8 * sig.D).
const INV_EIGHT = invert(8n, L);
...
const aggHash = (tag, P, Cnonzero, I, D, Cout) =>
  decodeInt(hashToScalar(concatBytes(tag, ...P, ...Cnonzero, I, D, Cout)));
 ...
 * @param {{ dest: Uint8Array, mask: Uint8Array }} inSk - real input secret key and commitment mask
 * @param {Uint8Array} a - 32-byte pseudoOut mask
...
export function proveClsag(message, pubs, inSk, a, Cout, index) {
  ...
  const p = decodeInt(inSk.dest);
  const z = mod(decodeInt(inSk.mask) - decodeInt(a), L);
  ...
  // nonce: aG = a*G, aH = a*H
  const alpha = decodeInt(randomScalar());
  ...
  const roundHash = (Lb, Rb) => decodeInt(hashToScalar(concatBytes(roundPrefix, Lb, Rb)));
  ...
  while (i !== index) {
    const si = decodeInt(randomScalar());
    s[i] = encodeInt(si);
    const cp = mod(muP * c, L);
    const cc = mod(muC * c, L);
  ...
  s[index] = encodeInt(mod(alpha - mod(c * mod(muP * p + muC * z, L), L), L));
...
export function verifyClsag(message, sig, pubs, Cout) {
  try {
    ...
    const D = decodePoint(sig.D).multiplyUnsafe(8n);
    if (D.is0()) {
      return false;
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      c = decodeInt(hashToScalar(concatBytes(roundPrefix, encodePoint(Lp), encodePoint(Rp))));
      if (c === 0n) {
        return false;
      }
    }
    return mod(c - c1, L) === 0n;
  } catch {
    return false;
  }
}
