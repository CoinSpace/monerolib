...
function _readTxPrefixOutputs(buf, outputsLength) {
  ...
  for (let i = 0; i < outputsLength; i++) {
    const amount = decodeVarint(buf.slice(start));
    start += amount.length; // amount
    const outputType = decodeVarint(buf.slice(start));
    start += outputType.length; // outputType
    if (outputType.number === 2) { // cryptonote::txout_to_key
      start += 32;
    } else if (outputType.number === 3) { // cryptonote::txout_to_tagged_key
      start += (32 + 1); // view tag
    } else {
...
function _getTxBase(buf, inputsLength, outputsLength) {
  ...
  if (type === RCTTypes.Bulletproof2 || type === RCTTypes.CLSAG || type === RCTTypes.BulletproofPlus) {
    i += 8 * outputsLength;
  } else {
    i += 64 * outputsLength;
  }
 ...
 * @returns {{ txPubKey: Uint8Array, encryptedPaymentId: Uint8Array, additionalPubKeys: Uint8Array[] }}
 */
...
export function parseTxExtra(buf) {
  ...
  let txPubKey = new Uint8Array(32);
  let encryptedPaymentId = new Uint8Array(0);
  const additionalPubKeys = [];
  for (let i = 0; i < buf.length; i++) {
    const item = buf[i];
    const tag = tags.find((tag) => tag === item);
    // best-effort extraction: stop at the first unknown tag instead of failing like monero
    ...
    if (tag === TX_EXTRA_TAG_PUBKEY) {
      ...
      // monero encodes counts and sizes as varint; here they are read as a single byte,
      // which holds because every count/size in tx_extra is always < 128 in practice
      const num = buf[i + 1];
      if (isNaN(num)) break;
      i += 2;
      for (let j = 0; j < num; j++) {
        additionalPubKeys.push(buf.slice(i, i + 32));
        i += 32;
      }
      i--;
  ...
  return { txPubKey, encryptedPaymentId, additionalPubKeys };
}
