 ...
 * @param {bigint} viewSecretKey - sender view secret scalar (for change outputs)
 ...
 *
 * @param {object} params
 * @param {{ secretKey: bigint, amount: bigint, mask: bigint, ring: { publicKey: Uint8Array, commitment: Uint8Array, globalIndex: bigint }[] }[]} params.inputs
 ...
 * @param {bigint} [params.viewSecretKey] - sender view secret scalar, required if any output is change
 * @param {Uint8Array} [params.paymentId] - 8-byte short payment id (integrated address)
 * @param {bigint} [params.unlockTime]
 * @param {bigint} [params.txSecretKey] - tx secret scalar r (random if omitted)
...
export function createTransaction({
  ...
  const ecdhInfo = outputs.map((o, i) => {
    const tuple = { amount: encodeInt(o.amount) };
  ...
  const sumOutMask = masks.reduce((s, m) => Fn.add(s, m), 0n);
  const pseudoMasks = [];
  let sumPseudo = 0n;
  ...
  const pseudoOuts = ins.map((inp, i) => pedersenCommitment(inp.amount, pseudoMasks[i]));
  ...
  const CLSAGs = ins.map((inp, i) => {
    const { s, c1, D } = proveClsag(message, inp.pubs, inp.inSk, pseudoMasks[i], pseudoOuts[i], inp.realIndex);
    return { s, c1, D };
  });
