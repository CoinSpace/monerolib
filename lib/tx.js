...
import { MAX_COMMITMENTS, RCTTypes } from './config.js';
import {
  TX_EXTRA_NONCE,
 ...
 * @property {bigint} amount
 * @property {boolean} [isChange] - the sender's own change output (derived via a*R)
 ...
 * @param {import('./bulletproofs.js').BulletproofPlus} bulletproofPlus
 ...
 * @param {bigint} secretViewKey - sender view secret scalar (for change outputs)
 * @returns {{
 *   txPublicKey: Uint8Array,
 *   additionalPublicKeys: Uint8Array[],
...
export function generateOutputs(destinations, txSecretKey, secretViewKey) {
  ...
  const outputs = destinations.map((destination, i) => {
    ...
    let derivation;
    if (destination.isChange) {
      derivation = generateKeyDerivation(txPublicKey, secretViewKey); // a*R
    } else {
      const ephemeralSecretKey = (destination.type === 'subaddress' && needAdditional)
        ? additionalSecretKey : txSecretKey;
      derivation = generateKeyDerivation(destination.publicViewKey, ephemeralSecretKey); // r*A (or s*C for subaddress)
    }
    return {
  ...
  return { txPublicKey, additionalPublicKeys, outputs };
}
...
function destinationViewKeyForPid(destinations) {
  ...
  for (const destination of destinations) {
    if (destination.isChange) {
      changeViewKey = destination.publicViewKey;
      continue;
    }
    if (destination.amount === 0n) {
      continue;
    }
    const samePicked = picked && equalBytes(picked.publicSpendKey, destination.publicSpendKey)
      && equalBytes(picked.publicViewKey, destination.publicViewKey);
    if (samePicked) {
      continue;
    }
    if (picked) {
  ...
  return picked ? picked.publicViewKey : changeViewKey;
}
...
function prepareInputs(inputs) {
  return inputs.map((input) => {
    ...
    const real = { publicKey, commitment: input.commitment, globalIndex: input.globalIndex };
 ...
 * output_j: mask_j = Hs(amount_key_j), V_j = mask_j*G + amount_j*H
...
export function prepareTransaction({
  ...
  if (outputs.length < 1 || outputs.length > MAX_COMMITMENTS) {
    throw new Error('createTransaction: invalid number of outputs');
  }
  ...
  const built = generateOutputs(outputs, txSecretKey, secretViewKey);
  const masks = built.outputs.map((output) => genCommitmentMask(output.amountKey));
  const { proof, V: outPk } = proveRangeBulletproofPlus(
    built.outputs.map((output, i) => ({ amount: output.amount, mask: masks[i] }))
  );
  const ecdhInfo = built.outputs.map((output) => (
    ecdhEncode({ amount: encodeInt(output.amount) }, output.amountKey, RCTTypes.BulletproofPlus).amount.slice(0, 8)
  ));
  // txout_to_tagged_key (0x03); amount is zeroed for rct
  const vout = built.outputs.map((output) => ({
    amount: 0n, target: { TAG: 0x03, data: { key: output.key, viewTag: output.viewTag } },
  }));

  // an integrated-address recipient embeds its payment id; otherwise a dummy short one for the
  // usual <= 2 output transfer (monero pads every small transfer with an encrypted payment id)
  ...
  if (integrated) {
    ...
    encryptedPaymentId = encryptPaymentId(integrated.paymentID, viewKeyForPid, txSecretKey);
  ...
  const extra = buildTxExtra({
    ...
    additionalPublicKeys: built.additionalPublicKeys,
    encryptedPaymentId,
  });
  ...
  const vin = preparedInputs.map((input) => ({
    TAG: 0x02,
    data: { amount: 0n, keyOffsets: input.keyOffsets, keyImage: input.keyImage },
  }));
  ...
  const rctSigBase = { type: RCTTypes.BulletproofPlus, txnFee: fee, ecdhInfo, outPk };
  const message = getPreMlsagHash(prefixHash, rctSigBase, proof);
  const CLSAGs = preparedInputs.map((input, i) => {
    const { s, c1, D } = proveClsag(message, input.ring, pseudoOuts[i], {
  ...
  return { prefix, rctSigBase, rctSigPrunable: { bulletproofsPlus: [proof], CLSAGs, pseudoOuts } };
}
