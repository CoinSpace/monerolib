 ...
 * @property {Uint8Array} publicKey - the output one-time public key P; signing checks (keyOffset + b)*G == P
 ...
 * @property {bigint[]} outputOffsets
 */

/**
 * @typedef {object} TxKeys
 * @property {bigint} txSecretKey
 * @property {Uint8Array} txPublicKey
 * @property {bigint[]} additionalTxSecretKeys - index i matches the additional public key and vout[i] after output shuffling
 ...
 * @property {bigint} secretSpendKey - sender spend secret scalar b; each input's one-time secret is keyOffset + b
 * @property {bigint} [secretViewKey] - sender view secret scalar, required if any output is change
 * @property {bigint} [unlockTime]
 * @property {Partial<Pick<TxKeys, 'txSecretKey' | 'additionalTxSecretKeys'>>} [txKeys] - omitted secret keys are generated when needed
 * @property {boolean} [shuffleOutputs=true] - randomize the output order (shuffle_outs); false keeps the caller order
 */

/**
 * @typedef {object} TxExtra - the tx_extra fields to build
 * @property {Uint8Array} txPublicKey
 * @property {Uint8Array[]} [additionalTxPublicKeys]
...
export function getTxId(bytes) {
  const tx = fullTransaction.decode(bytes);
  ...
  const base = fastHash(rctBaseCoder(inputs, outputs).encode(tx.rctSigBase));
  const prunable = type === RCTTypes.Null
    ? new Uint8Array(32)
    : fastHash(rctPrunableCoder(type, inputs, outputs, mixin).encode(tx.rctSigPrunable));
...
export function buildTxExtra({
  ...
  if (additionalTxPublicKeys.length) {
    fields.push({ tag: TX_EXTRA_TAG_ADDITIONAL_PUBKEYS, data: additionalTxPublicKeys });
  }
...
/**
 * @typedef {object} ParsedTxExtra - the recognized tx_extra fields
 * @property {Uint8Array[]} txPublicKeys - every tx public key field, in order
 * @property {Uint8Array[]} additionalTxPublicKeys
 * @property {Uint8Array} [encryptedPaymentId] - 8 bytes
 */

/**
 * Extracts tx public keys, additional pub keys and encrypted payment id from tx_extra.
 ...
 * @returns {ParsedTxExtra}
 */

export function parseTxExtra(buf) {
  // every tx public key field is kept: scanTransaction tries each one; other fields: first one wins
  ...
  return txExtra.decode(buf).reduce((acc, { tag, data }) => {
    if (tag === TX_EXTRA_TAG_PUBKEY) {
      acc.txPublicKeys.push(data);
    } else if (tag === TX_EXTRA_TAG_ADDITIONAL_PUBKEYS && !acc.additionalTxPublicKeys.length) {
      acc.additionalTxPublicKeys = data;
    } else if (tag === TX_EXTRA_NONCE && !acc.encryptedPaymentId
      && data.length === 9 && data[0] === TX_EXTRA_NONCE_ENCRYPTED_PAYMENT_ID) {
      acc.encryptedPaymentId = data.slice(1);
    }
    return acc;
  }, /** @type {ParsedTxExtra} */ ({
    txPublicKeys: [], additionalTxPublicKeys: [], encryptedPaymentId: undefined,
  }));
}
...
export function getPreMlsagHash(prefixHash, rctSigBase, bulletproofPlus) {
  ...
  const baseHash = fastHash(rctBaseCoder(0, rctSigBase.ecdhInfo.length).encode(rctSigBase));
  const {
    A, A1, B, r1, s1, d1, L, R,
  } = bulletproofPlus;
...
/**
 * @typedef {object} GeneratedOutputs
 * @property {GeneratedOutput[]} outputs
 ...
 * D_i = 8*r*A_i (standard); 8*r*C_i (single subaddress, no additional key); 8*s_i*C_i (additional
 * subaddress key); 8*a_sender*R (change)
 ...
 * @param {Partial<Pick<TxKeys, 'txSecretKey' | 'additionalTxSecretKeys'>>} [txKeys] - omitted secret keys are generated when needed; additional keys follow destination order
 * @param {bigint} [secretViewKey] - sender view secret scalar, required if any output is change
 * @returns {GeneratedOutputs}
 */
export function generateOutputs(
  destinations,
  { txSecretKey = randomScalar(), additionalTxSecretKeys } = {},
  secretViewKey
) {
  const {
  ...
  if (!needAdditional) {
    additionalTxSecretKeys = [];
  } else if (additionalTxSecretKeys === undefined) {
    additionalTxSecretKeys = destinations.map(() => randomScalar());
  } else if (additionalTxSecretKeys.length !== destinations.length) {
    throw new Error(
      `generateOutputs: additionalTxSecretKeys: expected ${destinations.length}, got ${additionalTxSecretKeys.length}`
    );
  }
  ...
  const additionalTxPublicKeys = [];
  const outputs = destinations.map((destination, i) => {
    let additionalSecretKey;
    if (needAdditional) {
      additionalSecretKey = additionalTxSecretKeys[i];
      additionalTxPublicKeys.push(destination.type === 'subaddress'
...
// shuffle_outs: the output position must not reveal the change
// https://github.com/monero-project/monero/blob/v0.18.5.0/src/cryptonote_core/cryptonote_tx_utils.cpp#L396-L399
...
/**
 * Prepare each spendable input for signing: reconstruct the one-time secret (keyOffset +
 * secretSpendKey), build its ring, and sort inputs by key image (descending) as monero orders vin.
 *
 * @param {TxInput[]} inputs
 * @param {bigint} secretSpendKey - sender spend secret scalar b
 * @returns {PreparedInput[]}
 */
function prepareInputs(inputs, secretSpendKey) {
  const prepared = inputs.map((input) => {
    const secretKey = Fn.add(input.keyOffset, secretSpendKey);
    const publicKey = secretKeyToPublicKey(secretKey);
    // reject a wrong spend key: xG must equal the scanned output key P (construct_tx_with_tx_key)
    // https://github.com/monero-project/monero/blob/v0.18.5.0/src/cryptonote_core/cryptonote_tx_utils.cpp#L371-L380
    if (!equalBytes(publicKey, input.publicKey)) {
      throw new Error('createTransaction: reconstructed key does not match the output (wrong spend key)');
    }
    const real = {
      publicKey,
    ...
    // https://github.com/monero-project/monero/blob/v0.18.5.0/src/cryptonote_core/cryptonote_core.cpp#L1036-L1049
    if (ring.some((member, i) => i > 0 && member.globalIndex === ring[i - 1].globalIndex)) {
      throw new Error('createTransaction: duplicate ring member');
    }
    return {
      secretKey,
      amount: input.amount,
      ...
      keyImage: generateKeyImage(publicKey, secretKey),
      outputOffsets: absoluteToRelative(ring.map((member) => member.globalIndex)),
    };
  }).sort((a, b) => compareBytesDesc(a.keyImage, b.keyImage));
  // key images must strictly descend: an input spent twice is rejected as "unsorted inputs"
  // https://github.com/monero-project/monero/blob/v0.18.5.0/src/cryptonote_core/blockchain.cpp#L3358-L3376
  if (prepared.some((input, i) => i > 0 && equalBytes(input.keyImage, prepared[i - 1].keyImage))) {
    throw new Error('createTransaction: duplicate input');
  }
  return prepared;
}
 ...
 * transaction object and its keys (use createTransaction to get the serialized bytes).
 * Math:
...
export function prepareTransaction({
  ...
  const destinations = shuffleOutputs ? shuffle(outputs) : outputs;
  const generated = generateOutputs(destinations, txKeys, secretViewKey);
  ...
  const integrated = destinations.find((output) => output.type === 'integratedaddress');
  if (integrated) {
    encryptedPaymentId = encryptPaymentId(integrated.paymentID, integrated.publicViewKey, generated.txKeys.txSecretKey);
  } else if (destinations.length <= 2) {
    // monero-oxide's rule, a deliberate simplification of monero's get_destination_view_key_pub, which
    // writes no nonce at all for two distinct recipients without change: with the dummy, one recipient
    // sees a random payment id; without a nonce, the whole chain sees a non-standard 2-output tx.
    // https://github.com/monero-project/monero/blob/v0.18.5.0/src/cryptonote_core/cryptonote_tx_utils.cpp#L219-L239
    // https://github.com/monero-oxide/monero-oxide/blob/731657ae3385be667abb556266369a497bc86f13/monero-oxide/wallet/src/send/tx.rs#L84-L96
    const target = destinations.find((output) => !output.isChange) ?? destinations.find((output) => output.isChange);
    encryptedPaymentId = encryptPaymentId(new Uint8Array(8), target.publicViewKey, generated.txKeys.txSecretKey);
  }
  const extra = buildTxExtra({
    txPublicKey: generated.txKeys.txPublicKey,
    additionalTxPublicKeys: generated.txKeys.additionalTxPublicKeys,
    encryptedPaymentId,
  });

  // inputs: ring assembled + sorted, key images, ordered by key image
  const preparedInputs = prepareInputs(inputs, secretSpendKey);
  /** @type {import('./raw.js').Vin[]} */
...
/**
 * Build, sign and serialize a transaction. Return the bytes and its keys.
 *
 * @param {TransactionParams} params
 * @returns {CreatedTransaction}
 */
export function createTransaction(params) {
  const { transaction, txKeys } = prepareTransaction(params);
  return {
 ...
 *
 * @param {bigint[]} outputOffsets - relative output-index offsets (monero's key_offsets field)
 * @returns {bigint[]} absolute global output indexes
 */
export function globalIndexesFromOutputOffsets(outputOffsets) {
  const MASK = (1n << 64n) - 1n;
  const globalIndexes = outputOffsets.slice();
 ...
 * https://github.com/monero-project/monero/blob/v0.18.5.0/src/wallet/wallet2.cpp#L8603-L8630
 * https://github.com/monero-project/monero/blob/v0.17.2.0/src/wallet/wallet2.cpp#L328
 *
 * @param {Number} weight
 * @param {bigint} baseFee - per-byte fee for the chosen priority, atomic units
 * @param {bigint} feeMultiplier - 1n; since 2021 scaling the priority is in baseFee, not in a multiplier
 ...
 * @param {Number} extra
 * @param {bigint} baseFee - per-byte fee for the chosen priority, atomic units
 * @param {bigint} feeMultiplier - 1n, see calculateFeeFromWeight
 * @param {bigint} feeQuantization - fee quantization mask, e.g. 10000; must be > 0
 * @param {Boolean} bulletproof
 * @param {Boolean} clsag
 * @param {Boolean} bulletproofPlus
