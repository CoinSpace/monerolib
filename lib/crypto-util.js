...
export function hashToPoint(data) {
  ...
  // check = w - x^2 * t
  let check = Fp.sub(w, Fp.mul(Fp.sqr(x), t));

  if (!Fp.is0(check)) {
    // check = w + x^2 * t
    check = Fp.add(w, Fp.mul(Fp.sqr(x), t));
    if (!Fp.is0(check)) {
      negative = true;
    } else {
      // x = x * fe_fffb1
  ...
  if (!negative) {
    ...
    r = Fp.neg(A);
    // check = w - sqrtm1 * x^2 * t
    check = Fp.sub(w, Fp.mul(Fp.mul(Fp.sqr(x), t), sqrtm1));
    if (!Fp.is0(check)) {
      // check = w + sqrtm1 * x^2 * t
      check = Fp.add(w, Fp.mul(Fp.mul(Fp.sqr(x), t), sqrtm1));
      if (!Fp.is0(check)) {
  ...
  const iz = Fp.inv(z);
  return Point.fromAffine({ x: Fp.mul(x, iz), y: Fp.mul(y, iz) });
}
