...
/**
 * xor8
 * https://github.com/monero-project/monero/blob/v0.17.1.9/src/ringct/rctOps.cpp#L683
 */
export function xor8(buf1, buf2) {
  const length = Math.max(buf1.length, buf2.length);
  const xor = (new BN(buf1)).xor(new BN(buf2)).toBuffer();
  return Buffer.concat([Buffer.alloc(length - xor.length), xor]);
}

/**
 * sc_sub
 * https://github.com/monero-project/monero/blob/v0.17.1.9/src/crypto/crypto-ops.c#L2687
 */
export function scSub(a, b) {
  const aa = ec.decodeInt(a);
  const bb = ec.decodeInt(b);
  return encodeInt(aa.sub(bb).umod(ec.curve.n));
}
