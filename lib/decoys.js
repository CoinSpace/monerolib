...
// Decoys hide the real spend, so every draw comes from the CSPRNG, like monero's gamma_engine
// (crypto::rand per draw), not from Math.random.
// https://github.com/monero-project/monero/blob/v0.18.5.0/src/wallet/wallet2.h#L107-L113
import { randomFloat, randomInt } from './crypto.js';
...
function gaussian() {
  return Math.sqrt(-2 * Math.log(1 - randomFloat())) * Math.cos(2 * Math.PI * randomFloat());
}
...
export function gammaPicker(rctOffsets) {
  ...
  return function pick() {
    ...
    x = x > DEFAULT_UNLOCK_TIME ? x - DEFAULT_UNLOCK_TIME : randomInt(RECENT_SPEND_WINDOW);
    ...
    return nRct === 0 ? -1 : firstRct + randomInt(nRct);
  };
}
