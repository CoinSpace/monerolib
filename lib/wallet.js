...
/**
 * The receive derivations to try for an output, yielded lazily: from the tx public key and, if
 * present, the per-output additional tx public key (a subaddress output in a mixed-recipient tx uses
 * the latter). Each derivation is D = 8 * secretViewKey * R for that public key R. A malformed key is
 * skipped, not thrown, so it never aborts a scan that would match on another derivation - matching
 * monero. Mirrors recv_derivation + additional_recv_derivations.
 ...
 *
 * The effective tx public key R is either the primary tx public key or, for a subaddress output in a
 * mixed-recipient tx, the per-output additional tx public key, so both are tried.
 * From R: derivation D = 8 * secretViewKey * R, one-time secret x = Hs(D || index) + b + m
 * (b the wallet spend secret, m the subaddress secret adjustment, 0 for the main address), and the
 * key image I = x * Hp(P) for the one-time output public key P.
 * The output is ours iff some derivation reproduces the given key image.
 *
...
export function isOwnKeyImage(keys, output, keyImage) {
  ...
  for (const derivation of receiveDerivations(output.txPublicKey, output.additionalPublicKey, secView)) {
    const { keyImage: derived } = outputKeyImage(secView, secSpend, derivation, output.index, output.subaddress);
