...
export default class Wallet {
  ...
  getAddress() {
    return new Address(this.#publicSpendKey, this.#publicViewKey, false, this.#nettype);
  }

  getSubaddressSecretKey(major, minor) {
  ...
  getSubaddress(major = 0, minor = 0) {
    if (major === 0 && minor === 0) {
      ...
      const m = this.getSubaddressSecretKey(major, minor);
      const M = ec.curve.g.mul(decodeInt(m));
      const B = decodePoint(this.#publicSpendKey);
      const D = B.add(M);
      const C = D.mul(decodeInt(this.#secretViewKey));
      return new Address(encodePoint(D), encodePoint(C), true, this.#nettype);
    }
  }
}
