 ...
 * @property {Uint8Array} [additionalPublicKey] - the per-output additional key R, if any
 ...
 * @returns {{ amount: bigint, mask: bigint, commitment: Uint8Array } | undefined}
 */
...
export function scanOutput(keys, output, subaddresses, primaryDerivation) {
  const {
    outputKey, index, viewTag, txPublicKey, additionalPublicKey,
  } = output;
  const secView = decodeInt(keys.secretViewKey);
  ...
  function* derivations() {
    ...
    const additional = receiveDerivation(additionalPublicKey, secView);
...
export function scanTransaction(keys, decodedTx, subaddresses) {
  ...
  const secView = decodeInt(keys.secretViewKey);
  // the tx public key derivation (8*a*R) is shared by every output; compute it once, not per output
  const txDerivation = receiveDerivation(txExtra.txPublicKey, secView);
  const outputs = [];
  let paymentId;
  prefix.vout.forEach((vout, index) => {
    ...
    const owned = scanOutput(keys, candidate, subaddresses, txDerivation);
    if (!owned) {
      return;
    }
    if (paymentId === undefined && txExtra.encryptedPaymentId.length === 8) {
      paymentId = encryptPaymentId(txExtra.encryptedPaymentId, txExtra.txPublicKey, secView);
    }
...
export function isOwnKeyImage(keys, output, keyImage) {
  ...
  for (const publicKey of [output.txPublicKey, output.additionalPublicKey]) {
 ...
 * transaction object and its serialized hex.
 *
 * @param {object} params
 ...
 * @returns {{
 *   json: import('./raw.js').Transaction,
 *   hex: string,
 * }}
 */
export function createTransaction({
  ...
  return {
    json,
    hex: bytesToHex(transaction.encode(json)),
  };
}
