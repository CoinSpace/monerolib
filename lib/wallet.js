 ...
 * @property {Uint8Array} ecdhInfo - the per-output ecdh entry
 * @property {Uint8Array} outPk - the per-output commitment
 * @property {number} rctType
 */
...
export function scanOutput(keys, output, subaddresses) {
  ...
  for (const publicKey of [txPublicKey, additionalPublicKey]) {
    ...
    const ecdh = isV2EcdhType(rctType) ? { amount: ecdhInfo } : ecdhInfo;
    const { mask, amount } = decodeRct(ecdh, outPk, rctType, index, derivation);
    const owned = {
      subaddress: found,
      index,
      amount,
