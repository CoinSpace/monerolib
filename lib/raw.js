...
// txin_to_key: amount, key_offsets, k_image
...
export const txParams = (prefix) => ({
  ...
  mixin: prefix.vin[0].TAG === TXIN_TO_KEY_TAG ? prefix.vin[0].data.keyOffsets.length - 1 : 0,
});
  ...
  return P.wrap({
    encodeStream: (w, sigs) => {
      prefix.vin.forEach((vin, i) => {
        const size = vin.TAG === TXIN_TO_KEY_TAG ? vin.data.keyOffsets.length : 0;
        P.array(size, signature).encodeStream(w, sigs[i] ?? []);
      });
    },
    decodeStream: (r) => prefix.vin.map(
      (vin) => P.array(vin.TAG === TXIN_TO_KEY_TAG ? vin.data.keyOffsets.length : 0, signature).decodeStream(r)
    ),
  });
}
