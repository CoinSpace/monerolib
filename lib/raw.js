...
import { RCTTypes, isV2EcdhType } from './ringct.js';

/**
 * LEB128 varint, as used everywhere in the monero wire format.
 * https://github.com/monero-project/monero/blob/v0.18.5.0/src/common/varint.h
 *
 * Returns a bigint, since amounts, fee and unlock_time are uint64.
 */
export const varint = P.wrap({
  encodeStream: (w, value) => {
    let v = BigInt(value);
    if (v < 0n || v > 0xffffffffffffffffn) {
      throw w.err(`varint: value out of uint64 range ${value}`);
    }
    while (v >= 0x80n) {
...
/** varint decoded as a JS number, for vector lengths and small counts. */
export const varintNum = P.apply(varint, P.coders.numberBigint);
 ...
 */
export const keyV = P.array(varintNum, key);
...
export const txinToKey = P.struct({
  amount: varint,
  keyOffsets: P.array(varintNum, varint),
  keyImage: key,
});

// txin_gen (tag 0xff): miner input, only the block height
export const txinGen = P.struct({ height: varint });
...
export const txout = P.struct({ amount: varint, target: txoutTarget });

export const txPrefix = P.struct({
  version: varintNum,
  unlockTime: varint,
  vin: P.array(varintNum, txin),
  vout: P.array(varintNum, txout),
  extra: P.bytes(varintNum),
});
...
export function rctBase(inputs, outputs) {
  return P.wrap({
    encodeStream: (w, v) => {
      ...
      varint.encodeStream(w, v.txnFee);
    ...
    decodeStream: (r) => {
      ...
      if (!RCT_TYPE_VALUES.has(type)) {
        throw r.err(`rctBase: unknown rct type ${type}`);
      }
      if (type === RCTTypes.Null) {
        return { type };
      }
      const res = { type, txnFee: varint.decodeStream(r) };
...
export function rctPrunable(type, inputs, outputs, mixin) {
  ...
  const rangeProofs = isBpPlus
    ? P.array(varintNum, bulletproofPlus)
    : (isBpFamily ? P.array(isBp ? P.U32LE : varintNum, bulletproof) : P.array(outputs, rangeSig));
