...
// P.tag types TAG as the literal variant key; widen it to number so hand-built prefixes (with
// plain-number tags) are assignable without a cast at the encode call.
export const txin = /**
 * @type {import('micro-packed').CoderType<{
 *   TAG: number,
 *   data: { amount: bigint, outputOffsets: bigint[], keyImage: Uint8Array } | { height: bigint }
 * }>}
 */ (P.tag(P.U8, {
...
export const txExtraFieldCoder = {
...
export const ecdhEntry = (type) => P.wrap({
 ...
 * @property {number} TAG
 * @property {{ amount: bigint, outputOffsets: bigint[], keyImage: Uint8Array }} data
 */
 ...
 * own outputOffsets (mixin+1) with no separate length prefix - txin_gen (coinbase) contributes a
 ...
 */
export function txSignatures(prefix) {
  return P.wrap({
    encodeStream: (w, sigs) => {
...
/**
 * A transaction codec. Monero serializes with a `pruned` flag that guards the prunable part
 * (`if (!pruned)`): version 1's legacy ring signatures, and version >= 2's rctSigPrunable. A pruned
 * codec stops after the base (prefix, plus rctSigBase for v2) — the base is all wallet.scanTransaction
 * needs, and it is what a pruned node / `/getblocks.bin?prune=true` serves. rctBase/rctPrunable are
 * factory coders parameterized by counts from the prefix (the rct arrays carry no length prefix).
 * https://github.com/monero-project/monero/blob/v0.18.5.0/src/cryptonote_basic/cryptonote_basic.h#L242-L322
 *
 * @param {boolean} pruned - drop the prunable part
 */
export function transactionCoder(pruned) {
  return P.wrap({
    ...
    decodeStream: (r) => {
      ...
      if (prefix.version === 1) {
        return pruned ? { prefix } : { prefix, signatures: txSignatures(prefix).decodeStream(r) };
      }
      if (prefix.version !== 2) {
...
export const transaction = transactionCoder(false);
