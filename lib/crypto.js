 ...
 *
 * Notation used across this module:
 *   p   secret key (scalar)
 *   P   public key (point)
 *   pV  secret view key
 *   PV  public view key
 ...
 *   PS  public spend key
 *   r   tx secret key
 *   R   tx public key (txPublicKey)
 *   X   stealth / one-time output key (targetKey)
 *   x   secret key of the stealth address
 *   J   key image (keyImage)
 *   Hs() hash to scalar (hashToScalar)
 *   Hp() hash to point (hashToEc)
 *   G   base point
 *
 * Key derivation (generateKeyDerivation):
 *   derivation = 8 * p * P
 *
 * Subaddress {account, minor i}:
 *   pSi = pS + Hs(pV | i)
 *   pVi = pV * pSi
 *
 *   PSi = PS + Hs(pV | i) * G
 ...
 *
 * Output tx key:
 *   address     R = r * G      (R is a public key)
 *   subaddress  R = r * PSi    (R is public data, not a key)
 *
 * Output stealth key, sending:
 *   address     X = Hs(r * PV | i) * G + PS
 *   subaddress  X = Hs(r * PVi | i) * G + PSi
 *
 * Output stealth key, receiving:
 *   address     X = Hs(R * pV | i) * G + PS
 *   subaddress  X = Hs(R * pV | i) * G + PSi
 *
 * Sending (r * PV) and receiving (R * pV) derive the same X, because r * PV = R * pV:
 *   address     R * pV = r * G * pV = r * PV
 *   subaddress  R * pV = r * PSi * pV = r * PVi
 *
 * Inputs:
 *   x = Hs(r * PV | i) + pS
 *   x = Hs(R * pV | i) + pS
...
/**
 * @typedef {import('@noble/curves/abstract/edwards.js').EdwardsPoint} Point - a decoded ed25519 point
 */
...
/**
 * Uniform random integer in [0, max), from the same source as randomScalar.
 * Values at or above the largest multiple of max are rejected, so the modulo is unbiased.
 *
 * @param {number} max - upper bound, 1 <= max <= 2^32
 * @returns {number}
 */
export function randomInt(max) {
  const limit = 2 ** 32 - (2 ** 32 % max);
  for (;;) {
    const num = Number(decodeInt(random(4)));
    if (num < limit) {
      return num % max;
    }
  }
...
/**
 * Uniform double in [0, 1) with 53 random bits, from the same source as randomScalar.
 * Same construction as std::generate_canonical<double> over a 64-bit engine.
 *
 * @returns {number}
 */
export function randomFloat() {
  return Number(decodeInt(random(8)) >> 11n) / 2 ** 53;
}
 ...
 * @returns {KeyPair} reduced secret scalar and public key
 */
...
/**
 * @typedef {object} SubaddressPublicKeys
...
/**
 * The view-only key offset for a received output: Hs(D || index) + m (m the subaddress secret, 0 for
 * the main address). Not spend-capable alone; the one-time secret is x = keyOffset + spend secret.
 * https://github.com/monero-oxide/monero-oxide/blob/946ec5f00ff071b129758ee8cba5528539fccfe4/monero-oxide/wallet/src/output.rs#L130-L134
 *
 * @param {bigint} secView - secret view scalar a
 * @param {Uint8Array} derivation - 32-byte shared secret (8*a*R / 8*r*A)
 * @param {number} index - output index in the tx
 * @param {SubaddressIndex} subaddress
 * @returns {bigint} the key offset scalar
 */
export function outputKeyOffset(secView, derivation, index, { major, minor }) {
  const offset = derivationToScalar(derivation, index);
  if (major === 0 && minor === 0) {
    return offset;
  }
  return Fn.add(offset, subaddressSecret(secView, { major, minor }));
}

/**
 * generate_key_image_helper_precomp: the key image of a received output.
 * The one-time secret x = keyOffset + b (see outputKeyOffset); key image I = x * Hp(x*G).
 ...
 * @param {number} index - output index in the tx
 * @param {SubaddressIndex} subaddress
 * @returns {Uint8Array} the 32-byte key image
 */
export function outputKeyImage(secView, secSpend, derivation, index, subaddress) {
  const secret = Fn.add(outputKeyOffset(secView, derivation, index, subaddress), secSpend);
  return generateKeyImage(secretKeyToPublicKey(secret), secret);
}
