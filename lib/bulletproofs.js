 ...
 * @param {bigint[]} masks - commitment masks
...
export function proveRangeBulletproofPlus(amounts, masks) {
  ...
  const Vreal = amounts.map((a, i) => Point.BASE.multiplyUnsafe(masks[i]).add(H.multiplyUnsafe(a)));
  ...
  for (let j = 0; j < amounts.length; j++) {
    alpha = Fn.add(alpha, Fn.mul(Fn.mul(ah.zPow[j], masks[j]), ah.yMnPlus1));
  }
...
function ipVerify(acc, ipRows, transcript, weight, proof, hBoldWeights, u) {
  ...
  const challenges = [];
  for (let i = 0; i < proof.L.length; i++) {
    const x = hashToScalar(encodeInt(t), proof.L[i], proof.R[i]);
    t = x;
    const xi = Fn.inv(x);
...
export function verifyBulletproof(proof, V) {
  try {
    ...
    let t = hashToScalar(...Vinv.map(encodePoint));
    const commitments = Vinv.map((P) => P.clearCofactor());

    const y = hashToScalar(encodeInt(t), proof.A, proof.S);
    const z = hashToScalar(encodeInt(y));
    t = z;
    const zPow = svPowers(z, 3 + padded);
    const x = hashToScalar(encodeInt(t), encodeInt(t), proof.T1, proof.T2);
    t = x;
    const xIp = hashToScalar(encodeInt(t), encodeInt(t), proof.taux, proof.mu, proof.t);
    t = xIp;
