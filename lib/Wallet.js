...
export default class Wallet {
  ...
  constructor({ seed, secretSpendKey, secretViewKey, publicSpendKey, nettype = 'mainnet' } = {}) {
    ...
    if (seed) {
      ...
      const { sec: secretViewKey, pub: publicViewKey } = generateKeys(fastHash(encodeInt(secretSpendKey)));

      this.#secretSpendKey = encodeInt(secretSpendKey);
      this.#publicSpendKey = publicSpendKey;

      this.#secretViewKey = encodeInt(secretViewKey);
      ...
      this.#publicSpendKey = secretKeyToPublicKey(decodeInt(secretSpendKey));
      this.#secretViewKey = secretViewKey;
      this.#publicViewKey = secretKeyToPublicKey(decodeInt(secretViewKey));
      ...
      this.#seed = encodeInt(secretSpendKey);
